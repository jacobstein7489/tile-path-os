# Frontend Reset + Commissions

## Foundation

- [ ] Commission plans / payments schema (additive only)
- [ ] Design tokens: warm canvas, layered depth, sizing scale
- [ ] Shared Popover / bottom-sheet primitive
- [ ] OwnerPicker (avatar + first name popover, optimistic)
- [ ] QuickDatePicker (Today / Tomorrow / weekday / next week / pick, optimistic)
- [ ] TaskRow (canonical, used everywhere)
- [ ] TaskDrawer operating panel + Move Forward outcome flow
- [ ] Capture: 2-step command modal (project -> what needs to happen)

## Navigation

- [ ] Sidebar: Today / Work / Projects / Schedule + secondary Deliveries / Commissions + Settings
- [x] Mobile bottom nav: Today / Work / Capture / Projects / Schedule + avatar sheet
- [ ] Remove Dashboard destination (redirect to /work)

## Screens

- [ ] Today (4 cards + real sections only)
- [ ] Work (4 cards, By Project | By Person)
- [ ] Projects card grid
- [x] Project shell: compact lifecycle rail + Overview/Tasks/Updates/Rooms/Schedule/Files + More (Tiles & Finishes, Install Materials, Deliveries, Field)
- [x] Project Overview command center
- [ ] Project Tasks
- [x] Project Updates timeline (real field reports)
- [ ] Update flow: Quick Update + Daily Field Update wizard
- [ ] Schedule (weekly desktop, day cards mobile)

## Commissions

- [ ] Settings > Commission Plans
- [ ] Salesperson + plan selection on award (snapshot)
- [ ] Commission Sheet + detail drawer
- [ ] Project > More > Commission

## Validation

- [ ] 1440x900, 1280x800, 390x844, 430x932 interaction tests + screenshots

## Sprint 1 — Setup & Design (approved)

- [ ] Migrations: finish_zone / finish_selection / finish_assignment, design meeting, question_rule, installer package + revisions, readiness_requirement, surface geometry, room plan ref
- [ ] Legacy surface spec backfill (default zone + selection + assignment, no guessing)
- [ ] Lifecycle reduced to 7 controlled stages (stage never auto-reverses)
- [ ] Rooms screen: surface + zone/assignment drawer editing
- [ ] Plan-assisted room setup (open plan, pick page, link room, optional pin)
- [ ] Office Setup derived summary on Overview
- [ ] Smart Design Meeting (allowlisted write handlers, one work item per unresolved)
- [ ] Installer Package publish + immutable revisions
- [ ] Deterministic readiness recomputation with named blockers
- [ ] Acceptance test on 114 Park Place (destructive writes on QA clone only)
