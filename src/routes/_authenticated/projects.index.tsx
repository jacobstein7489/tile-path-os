...
import {
  ProjectsWorkspaceV2,
  type ProjectQueueRecord,
} from "@/components/projects/ProjectsWorkspaceV2";
import {
  useProjects,
  ...
  head: () => ({
    meta: [
      { title: "Projects — Cobblestone Tile OS" },
      {
        name: "description",
        content:
          "Active tile projects, their next move, responsible crew, relevant date and attention reason.",
      },
      { property: "og:title", content: "Projects — Cobblestone Tile OS" },
      {
        property: "og:description",
        content: "Active projects and the operational facts that move them forward.",
      },
...
function ProjectsPage() {
  ...
  const activeCount = useMemo(
    () => projects.filter((project) => matchesView(project, "Active")).length,
    [projects],
  );
  const jobs = useMemo(
    () =>
...
function matchesView(project: Project, filter: ProjectView) {
  ...
  return (
    stage !== "Complete" &&
    !["On Hold", "Cancelled"].includes(project.exception_state ?? "")
  );
}
...
function matchesSearch(project: Project, search: string) {
  ...
  return (
    !query ||
    `${project.name} ${project.customer ?? ""} ${project.address ?? ""}`
      .toLowerCase()
      .includes(query)
  );
}
...
function relevantDateFor(project: Project, work: WorkItemRow[], schedule: ScheduleAssignment[]) {
  ...
  const dates = [
    ...work.flatMap((item) => [item.follow_up_on, item.due_date]),
    ...schedule.map((item) => item.work_date),
    project.start_date,
    project.target_date,
  ]
