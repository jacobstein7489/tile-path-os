...
export function WorkBoard({ projectId }: { projectId?: string }) {
  ...
  const customerName = (item: WorkItemRow) => {
    ...
    return (
      (project?.customer_company_id && companyMap.get(project.customer_company_id)) ||
      project?.customer ||
      "Company action"
    );
  };
  const scoped = useMemo(() => {
    const base = (projectId ? items.filter((i) => i.project_id === projectId) : items).filter(
      (i) => !isComplete(i),
    );
  ...
  const groups = useMemo(() => {
    if (mode === "All Actions")
      return [
        {
          key: "all",
          title: "All company actions",
          subtitle: "Every open operating action",
          rows: visible,
        },
      ];
          ...
          subtitle:
            mode === "By Project"
              ? first
                ? customerName(first)
                : "Customer not set"
              : `${new Set(rows.map(projectLabel)).size} projects`,
          rows,
        };
      })
      .sort(
        (a, b) =>
          b.rows.filter(isOverdue).length - a.rows.filter(isOverdue).length ||
  ...
  return (
    <OpsCanvas>
      <OpsPageHeader
        eyebrow="Company action center"
        title={projectId ? "Project Actions" : "Actions"}
        summary={
          isLoading ? "Loading actions…" : `${counts.Open} open actions · ${overdue} overdue`
        }
        action={
      ...
      >
        <div className="mt-5 grid grid-cols-2 gap-2 lg:grid-cols-5">
          {(
            [
              ["Open", counts.Open, FolderKanban],
              ["To Do", counts["To Do"], CheckSquare2],
              ["Waiting", counts.Waiting, UserRound],
              ["Scheduled", counts.Scheduled, CalendarCheck2],
              ["Overdue", overdue, AlertTriangle],
            ] as [Filter, number, typeof FolderKanban][]
            ...
            <button
              ...
              className={cn(
                "group flex min-h-16 items-center gap-3 rounded-xl border px-3 text-left transition-all",
                filter === label
                  ? "border-primary/30 bg-primary-soft shadow-[var(--shadow-card)]"
                  : "border-border bg-background/70 hover:-translate-y-0.5 hover:border-border-strong",
              )}
            >
              <ObjectMark
                tone={
              ...
              <span>
                <strong className="block text-[21px] leading-none tabular-nums">{value}</strong>
                <span className="mt-1 block text-[10px] font-bold uppercase text-muted-foreground">
                  {label === "Open" ? "All open" : label}
                </span>
              </span>
            </button>
          ))}
        ...
        <div className="mt-4 grid gap-2 lg:grid-cols-[auto_auto_minmax(220px,1fr)]">
          <Segmented
            values={projectId ? ["All Actions"] : ["By Project", "By Person", "All Actions"]}
            value={mode}
          ...
          />
          <label className="flex h-9 min-w-0 items-center gap-2 rounded-lg border border-border bg-background px-3 focus-within:ring-2 focus-within:ring-primary/20">
            ...
            return (
              <section key={group.key}>
                <button
                  ...
                  className="grid min-h-[82px] w-full grid-cols-[44px_minmax(0,1fr)_auto] items-center gap-3 px-4 text-left transition-colors hover:bg-primary-soft/25 sm:px-5"
                >
                  <ObjectMark tone={urgent ? "red" : "ink"}>
                    <FolderKanban className="size-5" />
                  </ObjectMark>
                  <span className="min-w-0">
                    <span className="flex min-w-0 items-center gap-2">
                      <strong className="truncate text-[16px]">{group.title}</strong>
                      <span className="ops-pill ops-pill-neutral">{group.rows.length}</span>
                      {urgent ? (
                        <span className="ops-pill ops-pill-red">{urgent} overdue</span>
                      ) : null}
                    </span>
                    <span className="mt-1 block truncate text-[11.5px] text-muted-foreground">
                      {group.subtitle} · Next: {nearest?.title ?? "No action"}
                    </span>
                  </span>
                  <ChevronDown
                    className={cn(
                ...
                {!closed ? (
                  <ul className="border-t border-border/60 bg-background/45 sm:px-3 sm:py-2">
                    {group.rows.map((item) => (
                      <OpsRow
                        key={item.id}
                        item={item}
                        ...
                        context={
                          mode === "All Actions"
                            ? [projectLabel(item), customerName(item), item.category]
                            : [
                                item.category,
                                item.waiting_on ? `Waiting on ${item.waiting_on}` : null,
                              ]
                        }
                        person={mode === "By Person" ? projectLabel(item) : ownerName(item)}
                        onOpen={setSelected}
                      />
                    ))}
                  </ul>
    ...
    <div className="flex h-9 items-center rounded-lg bg-muted p-1">
      {values.map((item) => (
        <button
          ...
          className={cn(
            "h-7 rounded-md px-3 text-[11.5px] font-bold",
            value === item
